<div class="content-wrap big-CTA-pre-footer">
    <!-- Call to action-->
    <div class="position-relative">
        <div class="half-section-block container position-absolute top-50 start-0 end-0" data-prev-id=".footerPrev" data-next-id=".footerNext">
            <div class="shape-parent">
                <div class="py-100 px-30 position-relative rounded-4 overflow-hidden z-index-1">
                    <div class="background">
                        <div class="background-image jarallax" data-jarallax data-speed="0.8"><img class="jarallax-img" loading="lazy" src="/assets/img/root/call-to-action-1920x1080.jpg" alt=""></div>
                        <div class="background-color" style="--background-color: #064175; opacity: .9;"></div>
                    </div>
                    <div class="row">
                        <div class="col-lg-8 offset-lg-2 text-center">
                            <img class="mb-50 logo w-sm-100" src="/images/header-logo-400px.png" alt="Hawaii Adventure Diving">
                            <h2 class="pb-40 text-white">Book <span class="hand-underline-yellow">Your</span> Adventure Today.</h2>
                            <!--<p class="mb-50 text-white px-lg-70" data-show-delay="100">If you are here for a truly once in a lifetime experience, then you are in the right place. Press the button below to book your tour today.</p>-->
                            <div>
                                <!-- Button-->
                                <?php echo "$btn"; ?>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!--// Call to action -->
</div>